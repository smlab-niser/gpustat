...
class GPUStat:
    ...
    def print_to(self, fp, *,
        ...
        # Memory
        _write(" | ")
        gpu_used_gb = round(safe_self.memory_used / 1024)
        gpu_total_gb = round(safe_self.memory_total / 1024)
        _write(rjustify(gpu_used_gb, 4), color='CMemU')
        _write(" / ")
        _write(rjustify(gpu_total_gb, 3), color='CMemT')
        _write(" GB")
        ...
        def process_repr(p: ProcessInfo):
            ...
            if show_pid:
                r += ("/%s" % _repr(p['pid'], '--'))
            gpu_gb = round(p['gpu_memory_usage'] / 1024) if p['gpu_memory_usage'] else 0
            r += '({CMemP}{}G{C0})'.format(gpu_gb, **colors)
            return r
...
class GPUStatCollection(Sequence[GPUStat]):
    ...
    def new_query(debug=False, id=None) -> 'GPUStatCollection':
        ...
        try:
            ...
            log.add_exception("system_memory", e)
            system_memory = None

        if debug:
            log.report_summary()

        return GPUStatCollection(gpu_list, driver_version=driver_version, system_memory=system_memory)
    ...
    def print_formatted(self, fp=sys.stdout, *,
                        ...
                        no_processes=False, show_ram=True,
                        eol_char=os.linesep,
                        ):
        ...
        if show_header:
            ...
            base_width = (gpuname_width or DEFAULT_GPUNAME_WIDTH) + 3  # len("[?]")

            header_template = '{t.black}{hostname:{width}}{t.normal}'

            if show_ram and self.system_memory:
                base_width = 12  # Reduced width to decrease space between hostname and CPU
                ram_used_gb = round(self.system_memory['used'] / (1024 ** 3))
                ram_total_gb = round(self.system_memory['total'] / (1024 ** 3))
                cpu_percent = self.system_memory['cpu_percent']

                # Define colors for readability
                grey = t_color.black
                bold_grey = t_color.bold_black
                yellow = t_color.yellow
                normal = t_color.normal

                # Build the RAM/CPU part of the header
                ram_cpu_part = (
                    f"{grey}CPU: {yellow}{cpu_percent:4.1f}{bold_grey}%"
                    f"{yellow}{ram_used_gb:>4}{normal}/"
                    f"{yellow}{ram_total_gb:>4}{grey} GB{normal} "
                )
                header_template += ram_cpu_part
            else:
                header_template += '  '
